...
def main():
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Quick and dirty 420chan Archiver")
        print("%s - Save the JSON and all images for an 420chan post." % (sys.argv[0]))
    ...
    board = py420chan.Board(board_name)
    thread = board.get_thread(thread_id)
    
    # create folders according to chan.arc standard
    path = os.path.join(os.getcwd(), "420chan", board_name, thread_id)
    images_path = os.path.join(path, "images")
    mkdirs(images_path)

    # archive the thread JSON
    url_builder = py420chan.Url(board_name)
